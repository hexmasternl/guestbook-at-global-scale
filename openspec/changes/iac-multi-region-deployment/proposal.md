...
- Add a **GitHub Actions workflow** that builds and pushes the API container image (passing the `MAXMIND_LICENSE_KEY` build secret to the existing `src/HexMaster.Guestbook.Api/Dockerfile`) and deploys the Bicep template via OIDC-federated Azure login. Because region fan-out lives in Bicep, the workflow itself needs no per-region changes.
...
- **New Azure resources**: 1 resource group, 1 Cosmos DB account (NoSQL, multi-region writes, session consistency, DB `guestbook` / container `entries` / PK `/region`), N Container Apps environments + Container Apps (one per enabled region), 1 Front Door Standard/Premium profile (endpoint, origin group, route), plus a user-assigned managed identity and one Cosmos DB data-plane role assignment. The container registry is an existing central ACR (not created here).
- **New dependencies / prerequisites**: an Azure subscription with an OIDC-federated app registration (GitHub → Azure), GitHub secrets for `AZURE_CLIENT_ID`/`AZURE_TENANT_ID`/`AZURE_SUBSCRIPTION_ID` and `MAXMIND_LICENSE_KEY`. No new NuGet/npm dependencies.
