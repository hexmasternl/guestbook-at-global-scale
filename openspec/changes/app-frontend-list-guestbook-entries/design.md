...
Requests carry a 15-second `timeout`. This was added after observing the failure directly: with the API stopped mid-session, the pending request neither resolved nor rejected — after nine seconds the page was still showing its spinner with `aria-busy="true"`. Without a deadline the error state is only reachable for failures that fail *fast* (a 500, a CORS rejection), not for a region that has simply stopped answering, which is exactly the scenario this talk is about demonstrating. An eternal spinner is the worst of the available outcomes on stage.

### 9. An explicit page size of 12, rather than the API's default

`loadPage` always sends `pageSize=12`. Letting the API apply its own default of 50 was the original intent — the client shouldn't duplicate a server-side constant — but it makes the feature inert at demo scale: verified against the running API, a guestbook of 18 entries came back as a single page with pagination permanently disabled, and the guestbook would need more than 50 greetings before a second page ever appeared. 12 also divides evenly across the one-, two- and three-column grid, and 50 cards is a long scroll on a phone. The value must stay inside the API's accepted 10–250 window or it is clamped server-side. This resolves the open question below about whether to expose a page-size control: the size is fixed, and the service method still takes `pageSize`, so a control remains easy to add later.
...
- [Nearest-centroid country lookup will be visibly wrong for some entries, especially near European borders or for small countries] → Mitigated by always showing the real coordinates next to the name and wording the label as approximate; a conference demo's entries come mostly from one room, where the country is right and the imprecision never surfaces. If it does become embarrassing, decision 4's rejected alternatives (polygons, or resolving server-side) are both open paths. **Two concrete misses were observed on seeded data and are locked in as tests** (`country-lookup.spec.ts`): Toronto resolves to "United States", Buenos Aires to "Uruguay". Toronto is the more awkward of the two — not a border town in the intuitive sense, but it sits deep inside the contiguous US longitude span and nearer the US centroid than Canada's. Both need real border polygons to fix. Amsterdam, Berlin, Paris, Rome, Madrid, Istanbul, Tromsø, Nairobi, Seattle, Anchorage and Honolulu all resolve correctly.
...
- ~~Whether the list should expose a page-size control~~ — resolved during implementation: fixed at 12, see decision 9.
- The API returns HTTP 500, not 400, for a malformed `continuationToken` (observed against the deployed endpoint with `?continuationToken=garbage`). The frontend only ever replays tokens the API itself returned, so it does not hit this, and any failure lands in the error state — but it is a backend robustness gap worth a separate change.
