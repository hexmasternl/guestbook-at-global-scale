...
- Show the handling data-center region — the DTO's `handledByRegion`, which is the backend instance that served the create request, not `region`, which is the Cosmos storage partition — as a friendly Azure region display name (`westeurope` → "West Europe"), falling back to the raw slug when unmapped and keeping the raw slug available on hover.
